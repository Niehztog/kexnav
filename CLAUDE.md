...
totals     : 41139 nodes, 162103 links, 36531 traversals, 546 nav edicts
...
* **A traversal is not extra geometry - for a ladder it restates its own two nodes.** `traversal.start` equals the source node's origin and `traversal.end` the target's, bit for bit, in all 272 of the corpus's `LADDER` links. That is the shape the whole climb has to be built to: BSPC reports a ladder as a chain of 64-unit chunks between mid-air areas, and snapping those to the nearest node put the traversal's start a median 50-80 units from where the bot stands, which is why bots could not climb. `kexnav/convert.py` groups the chain into one column per wall and emits **one** link, foot to top, both ends dedicated nodes on a floor, the foot flagged `LADDER` at radius 4 - all measured, all in [GENERATING](https://github.com/Niehztog/kexnav/wiki/GENERATING#ladders-one-climb-not-a-chain-of-rungs). Two traps if you touch it: the ladder plane AAS gives you points the *opposite* way to the one the corpus writes, and a `LADDER` area with no ground face must not get a node at all.
